...
class DeliveryService {
    async getPendingOrders() {
        try {
            const sql = "SELECT * FROM orders WHERE delivery_status = 'Ready' AND delivery_partner_id IS NULL";
    ...
    async getAssignedOrders(partnerId) {
        try {
            const sql = "SELECT * FROM orders WHERE delivery_partner_id = ? AND delivery_status != 'Delivered'";
    ...
    async acceptOrder(orderId, partnerId) {
        try {
            const sql = "UPDATE orders SET delivery_partner_id = ?, delivery_status = 'Accepted' WHERE order_id = ?";
    ...
    async updateLocation(partnerId, lat, lng) {
        try {
            ...
            const activeOrders = await Database.query("SELECT order_id FROM orders WHERE delivery_partner_id = ? AND delivery_status != 'Delivered'", [partnerId]);
